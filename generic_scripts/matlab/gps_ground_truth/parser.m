...
% Opens the file gpsLog.txt with read access.
    ...
    fileID = fopen('/media/sdrlaptop1/T7/15_09_2022_farm/gps_ground_truth _data/iphone_logger/2022-09-15 13_37_44.nmea','r');
% Read the text file.
...
lat = vertcat(ggaData.Latitude);
lon = vertcat(ggaData.Longitude);
alt = vertcat(ggaData.Altitude);

% Plot the position in geographic coordinates
geoplot([lat TX_lat], [lon Tx_lon],'Marker',"*",'MarkerSize',3, ...
...
for i=1:size(ggaData,1)
   [x y z] = latlonalt_xyz(lon(i),lat(i),alt(i));
   xyz(i,1) = x;
   xyz(i,2) = y;
   xyz(i,3) = z;
