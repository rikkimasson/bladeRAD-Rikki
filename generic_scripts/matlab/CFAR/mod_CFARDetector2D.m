classdef (Sealed,StrictDefaults) mod_CFARDetector2D < phased.internal.AbstractDetector
...
%#codegen
    ...
    methods
        function obj = mod_CFARDetector2D(varargin)
...
%             validateCUTS(obj,X,Idx);
            ...
            for m = 1:numCUT
                ...
                elseif obj.Method(1) == 'G' %'GOCA'
                    if m == 1  
                        ...
                        dIdx = Idx(:,m)-Idx(:,m-1);
                        trnindsl = trnindsl  + dIdx(1) + size(X,1)*dIdx(2);
                        trnindslg = trnindsl(trnindsl <= size(Idx,2) );
                        trnindsr = trnindsr  + dIdx(1) + size(X,1)*dIdx(2);
                        trnindsrg = trnindsr(trnindsr <= size(Idx,2) );
                    end
                    % Form noise power estimate
    ...
    methods (Access = protected)
        function varargout = get2DTrainingInds(obj,X,Idx)
            ...
            % truncate the training and guard cell indicies to 
            % when column or row index is > 1 
            
            indx1_r =  indx1_r( indx1_r > 0 );
            indx1_c = indx1_c( indx1_c > 0 );
            indx2_r = indx2_r( indx2_r > 0 );
            indx2_c = indx2_c( indx2_c > 0 );
            
            indx1_r =  indx1_r( indx1_r <= NumRows );
            indx1_c = indx1_c( indx1_c <= NumColumns );
            indx2_r = indx2_r( indx2_r <= NumRows );
            indx2_c = indx2_c( indx2_c <= NumColumns );
            ...
            if (obj.Method(1) == 'S') || (obj.Method(1) == 'G') %SOCA || GOCA
              ...
              RearTrainingCellIdx = repmat(RearTrainingCellIdx,1,size(chIdx,2)) + ...
                ...
                TrainingCellIndx = repmat(TrainingCellIndx, 1 , size(chIdx,2)) + repmat((chIdx-1)*(NumRows*NumColumns),size(TrainingCellIndx,1),1);
