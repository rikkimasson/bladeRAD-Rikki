function [signal, noise, SNR, slices_range_doppler, index_signal_max, noise_abs, slices_range_doppler_abs] = slice_SNR_abs_input(range_doppler_slice, roi_indices_rows, roi_indices_cols)

%   This function takes a range doppler slice as an absolute, not as at complex, 
%   converts the absolute values into signal power. 
%   Once this is known, for each slice, the Signal is identified
...
abs_2_power = 20*log10(range_doppler_slice); % [dB]
[val, idx] = max(abs_2_power,[],"all");
signal = val;  % [dB]
index_signal_max = idx;
noise = 20*log10(mean(roi_data(:))); % [dB]
noise_abs = mean(roi_data(:)); % absolute not dB
%noise = 20*log10(mean(complex_2_abs(:)));
SNR = signal - noise; % [dB]
slices_range_doppler = abs_2_power; % [dB]
slices_range_doppler_abs = range_doppler_slice; % [dB]
end
