...
% FMCW Parameters 
FMCW_Fs = 30e6;          % Sample Rate of SDR per I & Q (in reality Fs is double this)
pulse_duration = 1e-3;   % Desired Pulse Duration 
FMCW_Bw = 30e6;          % LFM Bandwidth 
FMCW_Fc = 5.75e9;   % Central RF 
...
passive_Fc = 2437e6;   % Central RF    
Ref_gain = 5; % 26dB seems good for C-Band Patch Antennas [-16, 60]
Sur_gain = 20; % 40dB seems good for C-Band Patch Antennas [-16, 60]
...
if process_active == true
    % load refsig for deramping
        refsig = load_refsig(FMCW_Bw_M,FMCW_Fs,FMCW_Fc,pulse_duration);    
    % load Signal, Mix and Dermap Signal  
        file_location = exp_dir + 'active_' + Experiment_ID;
        lp_filter = false;
        [max_range_actual,deramped_signal] = deramp_and_decimate(file_location,FMCW_max_range,refsig,capture_duration,FMCW_number_pulses,FMCW_Fs,slope,lp_filter);
